...
for (const e of events ?? []) { if (e.done && mode === "tomorrow") continue; const who = (e.assignee === "jake" || e.assignee === "victoria") ? e.assignee : "both"; const tag = (mode === "week" && who !== "both") ? " <em style='color:#8a7a68;font-size:12px;'>(" + whoName(who) + ")</em>" : ""; const dp = e.done ? "✓ <span style='text-decoration:line-through;opacity:.6;'>" : ""; const dsx = e.done ? "</span>" : ""; if (e.type === "gb_ship") { const s = GB_SUMMARY[e.package_key ?? ""] ?? e.title ?? "Goldbelly package"; const q = e.qty ?? 1; add(e.event_date, 0, dp + "<strong style='color:#c0392b;'>🚚 SHIP GOLDBELLY: " + q + " box" + (q > 1 ? "es" : "") + "</strong><br><span style='color:#7a6a58;font-size:13px;'>" + s + (q > 1 ? " - ship " + q + " of these" : "") + "</span>" + (e.title ? "<br><span style='color:#7a6a58;font-size:13px;'>📝 " + e.title + "</span>" : "") + dsx); } else if (e.type === "bake") { add(e.event_date, 2, dp + "<strong style='color:#8b6914;'>🍞 Bake day</strong>" + (e.title ? " - " + e.title : "") + tag + dsx, who); } else if (e.type === "media") { add(e.event_date, 3, dp + "<strong style='color:#7b5ea7;'>📸 Media</strong>" + (e.title ? " - " + e.title : "") + tag + dsx, who); } else if (e.type === "prospect") { add(e.event_date, 3, dp + "<strong style='color:#1f7a72;'>🤝 Potential partner drop/ship</strong>" + (e.title ? " - " + e.title : "") + tag + dsx, who); } else { add(e.event_date, 3, dp + "📝 " + (e.title || "Note") + tag + dsx, who); } }
